...
export default new Vuex.Store({
  ...
  mutations: { // Procesos que cambian los valroes de esos atributos generales
    ...
    updateMap(state) {
      ...
      let s = new XYZ({
          url: `http://${document.location.hostname}:${process.env.VUE_APP_BE_PORT}/${state.filterSelected}/${state.dateSelected}/{z}/{y}/{x}?key=${process.env.VUE_APP_BE_API_TOKEN}`,
          maxZoom: 9
      });
  ...
  actions: { //Eventos o acciones que llaman a los procesos anteriores.
    getFilters({commit}){
      ...
      .get(`http://${document.location.hostname}:${process.env.VUE_APP_BE_PORT}` + "/filters", {
          headers: {
    ...
    getMap({commit}){
        const mapX = new Map({
          ...
          layers: [
              ...
              new TileLayer({
                  source: new XYZ({
                      url: `http://${document.location.hostname}:${process.env.VUE_APP_BE_PORT}/${this.state.filterSelected}/${this.state.dateSelected}/{z}/{y}/{x}?key=${process.env.VUE_APP_BE_API_TOKEN}`,
                      maxZoom: 9
                  })
              }),
              new TileLayer({
